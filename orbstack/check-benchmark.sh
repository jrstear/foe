...
KUBECTL_ARGS="--context orbstack"

echo -e "${BLUE}🔍 Checking Flux Benchmark Status...${NC}"
echo ""
...
if [ -z "$POD_NAME" ]; then
  echo -e "${YELLOW}⚠ Flux MiniCluster not found.${NC}"
  exit 1
fi
...
# using 2>/dev/null to suppress "Defaulted container" warning if present in stderr
